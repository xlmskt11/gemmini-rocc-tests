// Two-Gemmini, full-local-memory waveform proxy for the same four logical
// matmuls used by llama2_ffn_gate_up_m8_2plus2_waveform.c.
//
// Each logical matmul uses physical Gemmini 0/1 together (mask 0x3) and is
...
#define WAVE_K 512u
#define WAVE_JOBS 4u
#define WAVE_MEMBERS 2u
#define WAVE_MASK 0x3u
#define WAVE_STEPS_PER_JOB 24u
...
#error "The g2-full waveform replay requires the shared partition descriptor ABI"
#endif
...
static wave_pair_desc_t wave_schedule[WAVE_TOTAL_STEPS]
...
static bool wave_prepare_job(
  ...
  // tile_id remains software metadata only. The launch group ID is derived
  // from the live ACC half so all K partials targeting the same allocation
  // share one LoopMatmul lifecycle group.
  ...
  if (!shared_multi_matmul_job_set_exact_tiling(
      ...
      job->tile_I != 1u || job->tile_J != 23u || job->tile_K != 22u ||
      job->sp_addr_A_stacked != 176u ||
      job->sp_addr_B_stacked != 4048u ||
      job->acc_addr_stacked != 184u)
    return false;
  ...
  return job->partition_status == SHARED_MULTI_PARTITION_OK &&
      ...
      job->I0 == 1u && job->J0 == 8u && job->K0 == 3u &&
      job->last_I == 1u && job->last_J == 11u && job->last_K == 20u &&
...
static bool wave_prepare_one_step(
    shared_multi_matmul_job_t *job, wave_pair_desc_t *descriptor) {
  ...
  for (size_t rank = 0; rank < WAVE_MEMBERS; ++rank) {
    ...
    if (step.K % WAVE_MEMBERS != 0u ||
        ...
        plan.aux_offset != rank || plan.partition_pad != 0u ||
        plan.aux_pad != 0u)
      return false;
...
static inline __attribute__((always_inline)) void wave_issue_pair(
    const wave_pair_desc_t *descriptor) {
  WAVE_ISSUE_MEMBER(custom0, &descriptor->member[0]);
  WAVE_ISSUE_MEMBER(custom1, &descriptor->member[1]);
  ROCC_INSTRUCTION_RS1_RS2(
      custom1, descriptor->launch.rs1, descriptor->launch.rs2, k_LOOP_WS);
}
...
// measured loop contains only descriptor loads, 17 RoCC issues, loop control,
...
static __attribute__((noinline)) wave_cycles_t wave_replay_schedule(void) {
  ...
  for (size_t step = 0; step < WAVE_TOTAL_STEPS; ++step)
    wave_issue_pair(&wave_schedule[step]);
...
int main(void) {
  if (DIM != 8 || BANK_NUM != 4 || BANK_ROWS != 4096 || ACC_ROWS != 8192 ||
      ...
      sizeof(wave_pair_desc_t) != 272u || !wave_ranges_are_valid()) {
    printf("WAVEFORM-G2-FULL-FAIL stage=geometry\n");
    return 1;
  }

  printf("WAVEFORM-G2-FULL-BEGIN shape=ffn_gate_up_scaled_1over8 "
         "source_shape=8x11008x4096 M=%u N=%u K=%u "
         "mask=0x3 gemminis=2 jobs=4 schedule=queued4 "
         "spad=full acc=full axis=K tile=1/23/22 steps_per_job=24 "
         "group_id=acc_half "
  ...
  if (!wave_prepare_schedule()) {
    printf("WAVEFORM-G2-FULL-FAIL stage=schedule_prepare\n");
    return 1;
  }
  if (!wave_configure_hw_once()) {
    printf("WAVEFORM-G2-FULL-FAIL stage=hardware_config\n");
    return 1;
  }
  wave_condition_cache();

  const wave_cycles_t cycles = wave_replay_schedule();
  printf("WAVEFORM-G2-FULL-RESULT start_cycle=%llu issue_cycles=%llu "
         "fence_cycles=%llu makespan_cycles=%llu steps=%u "
         "commands_per_step=17 cache_sink=%llu\n",
