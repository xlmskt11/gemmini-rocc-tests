...
#error "define VPU_ENABLE_GEMMINI_FLASHATTENTION=1 for the fusion kernel"
    ...
    !defined(ELEM_T_SIG_BITS) || ELEM_T_EXP_BITS != 8 || \
    ELEM_T_SIG_BITS != 8
...
#error "vpu_flashattention_kernel requires at least one matrix tile per VSRAM bank"
#endif
...
typedef struct {
  ...
  /* Element strides. A zero stride requests the packed logical width. */
  size_t query_stride;
  size_t key_stride;
  size_t value_stride;
...
#define VPU_FA_OUTPUT_BANK_BASE VPU_MATRIX_PORTS
#define VPU_FA_VSRAM_MATRIX_ROWS_PER_BANK (VPU_ELEMENTS_PER_BANK / DIM)
#define VPU_FA_CAUSAL_MASK_ELEMENTS \
  VPU_FLASHATTENTION_CAUSAL_MASK_ELEMENTS
#define VPU_FA_CAUSAL_MASK_BANK \
  (VPU_FA_OUTPUT_BANK_BASE + VPU_MATRIX_PORTS - 1u)
...
static inline int vpu_fa_group_last_member(unsigned group_list) {
  for (int i = 3; i >= 0; --i) {
    if (((group_list >> i) & 1u) != 0u) return i;
  }
  return -1;
}
...
static inline void vpu_fa_config_ld(unsigned custom, size_t stride,
  ...
  const uint64_t rs1 =
      ...
      ((uint64_t)DIM << 16) | (UINT64_C(1) << 8) |
      ((uint64_t)id << 3) | VPU_FA_CONFIG_LD;
...
#define VPU_FA_SHARED_LOOP_WS(                                             \
    ...
    acc_addr_start_, ex_I_, mv_K_, mv_pad_K_, laddrI_offset_,              \
    laddrK_offset_, I_, J_, K_, pad_I_, pad_J_, pad_K_, A_, B_, D_, C_,   \
    A_stride_, B_stride_, D_stride_, C_stride_, A_transpose_,              \
    B_transpose_, full_C_, low_D_, ex_accumulate_, act_,                   \
    a_from_vsram_, d_from_vsram_, c_to_vsram_, has_vpu_followup_)          \
  do {                                                                     \
    VPU_FA_GEMMINI_ISSUE(                                                  \
        custom_, acc_addr_start_,                                          \
        ((uint64_t)(sp_addr_end_) << 16) | (uint64_t)(sp_addr_start_),     \
        VPU_FA_K_LOOP_WS_CONFIG_SPADDR);                                   \
    VPU_FA_GEMMINI_ISSUE(                                                  \
        custom_,                                                           \
        ((uint64_t)(laddrK_offset_) << 16) |                               \
            (uint64_t)(laddrI_offset_),                                    \
        ((uint64_t)(mv_K_) << 32) | ((uint64_t)(mv_pad_K_) << 16) |       \
            (uint64_t)(ex_I_),                                             \
        VPU_FA_K_LOOP_WS_CONFIG_MV_BOUNDS_1);                              \
    VPU_FA_GEMMINI_ISSUE(                                                  \
        custom_,                                                           \
        ((uint64_t)(pad_K_) << 32) | ((uint64_t)(pad_J_) << 16) |         \
            (uint64_t)(pad_I_),                                            \
        ((uint64_t)(K_) << 32) | ((uint64_t)(J_) << 16) | (uint64_t)(I_), \
    ...
    if (vpu_fa_issue_group_run((unsigned)(custom_),                        \
      ...
      VPU_FA_GEMMINI_ISSUE(                                                \
          custom_, vpu_fa_run_rs1,                                        \
          ((uint64_t)(B_transpose_) << 1) | (uint64_t)(A_transpose_),      \
          VPU_FA_K_LOOP_WS);                                               \
    }                                                                      \
  } while (0)
...
static inline unsigned vpu_fa_popcount(unsigned mask) {
  unsigned count = 0u;
  for (; mask != 0u; mask >>= 1) count += mask & 1u;
  return count;
}
...
static inline void vpu_fa_group_command(unsigned group_id, bool last,
  ...
  vpu_issue_grouped(group_id, last,
                    vpu_micro_op(opcode, rd, rs1, rs2, rs3, funct1),
                    payload);
}

static inline void vpu_fa_group_write_gp(unsigned group_id, unsigned reg,
                                         uint32_t value) {
  vpu_fa_group_command(group_id, false, VPU_OP_C_WRITE_GP, reg,
                       0, 0, 0, 0, value);
}
...
static inline void vpu_fa_group_loop_start(unsigned group_id,
                                            unsigned loop_reg,
                                            uint32_t iterations) {
  vpu_group_loop_start(group_id, loop_reg, iterations);
}

static inline void vpu_fa_group_loop_end(unsigned group_id,
                                          unsigned loop_reg) {
...
static inline void vpu_fa_group_write_fp(unsigned group_id, unsigned reg,
                                         float value) {
  vpu_fa_group_command(group_id, false, VPU_OP_C_WRITE_FP, reg,
                       0, 0, 0, 0, vpu_float_to_bits(value));
}

static inline void vpu_fa_group_terminator(unsigned group_id) {
  vpu_fa_group_command(group_id, true, VPU_OP_S_MAX_FP,
                       VPU_FA_FP_TERMINATOR, VPU_FA_FP_TERMINATOR,
                       VPU_FA_FP_TERMINATOR, 0, 0, 0);
}
...
static inline void vpu_fa_group_vector(unsigned group_id, unsigned opcode,
                                       ...
                                       unsigned src1_or_fp) {
  vpu_fa_group_command(group_id, false, opcode, dst_gp, src0_gp,
                       src1_or_fp, 0, 0, 0);
}
...
static inline void vpu_fa_group_scalar(unsigned group_id, unsigned opcode,
                                       ...
                                       unsigned src1_fp) {
  vpu_fa_group_command(group_id, false, opcode, dst_fp, src0_fp,
                       src1_fp, 0, 0, 0);
}
...
static inline void vpu_fa_group_state_load_current(unsigned group_id,
                                                    unsigned dst_fp,
                                                    unsigned state_gp) {
  vpu_fa_group_command(group_id, false, VPU_OP_S_LOAD_STATE, dst_fp,
                       state_gp, 0, 0, 0, 0);
}
...
static inline void vpu_fa_group_state_store_current(unsigned group_id,
                                                     unsigned state_gp,
                                                     unsigned src_fp) {
  vpu_fa_group_command(group_id, false, VPU_OP_S_STORE_STATE, state_gp,
                       src_fp, 0, 0, 0, 0);
}

static inline vpu_flashattention_config_t vpu_fa_normalize_config(
    const vpu_flashattention_config_t *source) {
  vpu_flashattention_config_t config = *source;
  if (config.query_stride == 0u) config.query_stride = config.q_dim;
  if (config.key_stride == 0u) config.key_stride = config.k_dim;
  if (config.value_stride == 0u) config.value_stride = config.value_dim;
  if (config.output_stride == 0u) config.output_stride = config.value_dim;
  return config;
}

static inline unsigned vpu_fa_query_tiles_for_custom(
    unsigned q_tiles, unsigned target_custom, unsigned gemmini_mask) {
  const unsigned gemmini_count = vpu_fa_popcount(gemmini_mask & 0xfu);
  const unsigned owner_count =
      q_tiles < gemmini_count ? q_tiles : gemmini_count;
  if (owner_count == 0u ||
      ((gemmini_mask >> target_custom) & 1u) == 0u) return 0u;

  unsigned member_index = 0u;
  for (unsigned custom = 0u; custom < 4u; ++custom) {
    if (((gemmini_mask >> custom) & 1u) == 0u) continue;
    if (custom == target_custom) {
      if (member_index >= owner_count) return 0u;
      return q_tiles / owner_count +
          (member_index < q_tiles % owner_count ? 1u : 0u);
    }
    ++member_index;
  }
  return 0u;
...
static inline bool vpu_fa_mask_region_fits(unsigned q_tiles,
                                            ...
                                            unsigned gemmini_mask) {
  if (VPU_FA_CAUSAL_MASK_ELEMENTS > VPU_ELEMENTS_PER_BANK) return false;
  const unsigned mask_owner =
      VPU_FA_CAUSAL_MASK_BANK - VPU_FA_OUTPUT_BANK_BASE;
  const size_t owner_output_elements =
      (size_t)vpu_fa_query_tiles_for_custom(
          q_tiles, mask_owner, gemmini_mask) *
      value_tiles * VPU_FA_MATRIX_TILE_ELEMENTS;
  return owner_output_elements <=
      VPU_ELEMENTS_PER_BANK - VPU_FA_CAUSAL_MASK_ELEMENTS;
}

static inline unsigned vpu_fa_query_tile_owner(
    unsigned q_rows, unsigned row_tile, unsigned gemmini_mask,
    unsigned *local_tile) {
  const unsigned i_tiles = vpu_fa_ceil_div(q_rows, DIM);
  const unsigned gemmini_count = vpu_fa_popcount(gemmini_mask & 0xfu);
  const unsigned owner_count =
      i_tiles < gemmini_count ? i_tiles : gemmini_count;
  const unsigned tiles_per_owner = i_tiles / owner_count;
  const unsigned extra_tiles = i_tiles % owner_count;
  unsigned tile_start = 0u;
  unsigned member_index = 0u;

  for (unsigned custom = 0u; custom < 4u; ++custom) {
    if (((gemmini_mask >> custom) & 1u) == 0u) continue;
    const unsigned member_tiles =
        tiles_per_owner + (member_index < extra_tiles ? 1u : 0u);
    if (member_index < owner_count && row_tile < tile_start + member_tiles) {
      *local_tile = row_tile - tile_start;
      return custom;
    }
    tile_start += member_tiles;
    ++member_index;
    if (member_index == owner_count) break;
  }
  *local_tile = 0u;
  return 0u;
}

static inline uint32_t vpu_fa_distributed_matrix_address(
    bool output, unsigned q_rows, unsigned tile_columns, unsigned row,
    unsigned column_tile, unsigned gemmini_mask) {
  const unsigned row_tile = row / DIM;
  const unsigned row_in_tile = row % DIM;
  unsigned local_tile = 0u;
  const unsigned owner = vpu_fa_query_tile_owner(
      q_rows, row_tile, gemmini_mask, &local_tile);
  const unsigned bank =
      (output ? VPU_FA_OUTPUT_BANK_BASE : VPU_FA_SCORE_BANK_BASE) + owner;
  const unsigned tile = local_tile * tile_columns + column_tile;
  return VPU_BANK_BASE(bank) +
      tile * VPU_FA_MATRIX_TILE_ELEMENTS + row_in_tile * DIM;
}

static inline bool vpu_fa_tiling_fits(unsigned q_tiles,
                                      unsigned kv_tiles,
                                      unsigned value_tiles,
                                      unsigned gemmini_mask) {
  ...
  return score_elements <= VPU_ELEMENTS_PER_BANK &&
      ...
      vpu_fa_mask_region_fits(q_tiles, value_tiles, gemmini_mask) &&
      pv_a_rows <= max_operand_rows && pv_b_rows <= max_operand_rows &&
      pv_c_rows <= max_acc_rows &&
      q_rows <= VPU_FP_STATE_ENTRIES / 2u;
}
 ...
 * declared shape. */
static inline bool vpu_fa_element_extent_overflows(
    size_t rows, size_t stride, size_t columns) {
...
static inline vpu_flashattention_status_t vpu_fa_validate_config(
    const vpu_flashattention_config_t *config) {
  ...
  if (config->query_rows == 0u || config->sequence == 0u ||
      config->q_dim == 0u || config->k_dim == 0u ||
      config->value_dim == 0u || config->q_dim != config->k_dim ||
      config->query_base > config->sequence ||
  ...
  if (config->query_stride < config->q_dim ||
  ...
  if (config->causal_mask_workspace_elements <
      VPU_FA_CAUSAL_MASK_ELEMENTS) {
    return VPU_FLASHATTENTION_INSUFFICIENT_MASK_WORKSPACE;
  }
  if (vpu_fa_element_extent_overflows(
          config->query_rows, config->query_stride, config->q_dim) ||
      vpu_fa_element_extent_overflows(
          config->sequence, config->key_stride, config->k_dim) ||
      vpu_fa_element_extent_overflows(
          config->sequence, config->value_stride, config->value_dim) ||
      config->output_stride > UINT32_MAX ||
      (config->query_rows - 1u) >
          (UINT32_MAX - (config->value_dim - 1u)) / config->output_stride) {
    return VPU_FLASHATTENTION_ADDRESS_RANGE_OVERFLOW;
  }
  if (VPU_FA_CAUSAL_MASK_BANK >= VPU_VSPAD_BANKS ||
      2u * VPU_MATRIX_PORTS * VPU_FA_VSRAM_MATRIX_ROWS_PER_BANK > ACC_ROWS ||
...
static inline vpu_flashattention_status_t vpu_flashattention_make_plan(
    const vpu_flashattention_config_t *source,
    vpu_flashattention_plan_t *plan) {
  if (source == NULL || plan == NULL)
    return VPU_FLASHATTENTION_INVALID_ARGUMENT;
  const vpu_flashattention_config_t config =
      vpu_fa_normalize_config(source);
  const vpu_flashattention_status_t validation =
      vpu_fa_validate_config(&config);
  if (validation != VPU_FLASHATTENTION_OK) return validation;

  const unsigned gemmini_count =
      vpu_fa_popcount(config.gemmini_mask & 0xfu);
...
static inline bool vpu_fa_matmul_job_prepare(
  ...
  if (factors.tile_I != i_tiles || factors.tile_J != j_tiles ||
      factors.tile_K == 0u || factors.tile_K > total_k_tiles)
    return false;
  ...
  job->dim_j = dim_j;
  job->dim_k = dim_k;
  job->tile_i = (unsigned)factors.tile_I;
...
static inline void vpu_fa_matmul_job_init(vpu_fa_matmul_job_t *job) {
  ...
  for (unsigned custom = 0u; custom < 4u; ++custom) {
    if (((job->gemmini_mask >> custom) & 1u) == 0u) continue;
    vpu_fa_config_ex(custom, 1u, qk);
    vpu_fa_config_st(custom);
    vpu_fa_config_ld(custom, a_stride * sizeof(elem_t), 0u);
    vpu_fa_config_ld(custom, b_stride * sizeof(elem_t), 1u);
...
static inline bool vpu_fa_matmul_job_step(
    vpu_fa_matmul_job_t *job, unsigned *dispatch_sequence) {
  if (!job->initialized || job->done) return false;
  ...
  const bool final_step =
      job->next_k_tile + k_tiles == job->total_k_tiles;
  ...
  for (unsigned custom = 0u; custom < 4u; ++custom) {
    if (((group_list >> custom) & 1u) == 0u) continue;
    const unsigned this_i =
        i_base + (member_index < i_extra ? 1u : 0u);
    const unsigned this_k =
        k_base + (member_index < k_extra ? 1u : 0u);
    ...
    const bool d_from_vsram = !qk && !job->first_attention_block &&
        job->next_k_tile == 0u;
    const bool ex_accumulate = job->next_k_tile != 0u || d_from_vsram;
    const unsigned score_local_row =
        VPU_BANK_BASE(VPU_FA_SCORE_BANK_BASE + custom) / DIM;
    const unsigned output_local_row =
        VPU_BANK_BASE(VPU_FA_OUTPUT_BANK_BASE + custom) / DIM;
    const unsigned c_global_row_offset = i_offset * job->tile_j * DIM;
    const unsigned a_global_row_offset = i_offset * k_tiles * DIM;
    const unsigned acc_local_row = qk ? score_local_row : output_local_row;
    const unsigned acc_base = this_i != 0u
        ? acc_local_row - c_global_row_offset : acc_local_row;
    const unsigned a_local_base = qk ? vpu_fa_spad_start(group_id) :
        (this_i != 0u ? score_local_row - a_global_row_offset
                      : score_local_row);
    const unsigned d_local_base = this_i != 0u
        ? output_local_row - c_global_row_offset : output_local_row;

    const elem_t *a = NULL;
    const elem_t *b = NULL;
    if (qk) {
      const unsigned a_row = this_i != 0u
          ? job->q_start + i_offset * DIM : job->q_start;
      const unsigned b_col = this_k != 0u
          ? k_element_start + k_offset * DIM : k_element_start;
      a = config->queries +
          (size_t)a_row * config->query_stride + k_element_start;
      b = config->keys +
          (size_t)job->kv_start * config->key_stride + b_col;
    } else {
      const unsigned b_row = this_k != 0u
          ? job->kv_start + k_element_start + k_offset * DIM
          : job->kv_start + k_element_start;
      b = config->values + (size_t)b_row * config->value_stride;
    }

    VPU_FA_SHARED_LOOP_WS(
        custom, group_list, group_id,
        a_local_base, vpu_fa_spad_end(group_id), acc_base,
        this_i, this_k, this_pad_k, i_offset, k_offset,
        job->tile_i, job->tile_j, k_tiles,
        this_pad_i, pad_j, pad_k,
        a, b,
        d_from_vsram ? (void *)(uintptr_t)d_local_base : NULL,
        NULL,
        qk ? config->query_stride : job->dim_k,
        qk ? config->key_stride : config->value_stride,
        qk ? 0u : config->value_dim, 0u,
        false, qk, false, false, ex_accumulate,
        VPU_FA_NO_ACTIVATION,
        a_from_vsram, d_from_vsram, final_step, final_step);
...
static inline void vpu_fa_group_score_max_stream(
...
static inline void vpu_fa_group_score_exp_stream(
    unsigned group_id, unsigned full_tiles, unsigned tail_columns) {
  const bool expand_tiles = VPU_LOOP_BUFFER_ENTRIES >= 56u &&
      tail_columns == 0u && full_tiles <= 4u;
  const unsigned total_columns = full_tiles * DIM + tail_columns;
...
static inline void vpu_fa_group_output_scale_stream(
    unsigned group_id, unsigned scalar_fp, unsigned value_dim) {
...
static inline void vpu_fa_issue_causal_mask_row_run(
    const vpu_flashattention_config_t *config,
    vpu_flashattention_stats_t *stats, unsigned group_id,
    unsigned q_rows, unsigned kv_start, unsigned kv_rows,
    unsigned first_row, unsigned row_count, unsigned global_query) {
  ...
  const bool all_visible = !all_future &&
      global_query >= kv_start + kv_rows - 1u;
  if (all_visible) return;

  const bool has_diagonal = !all_future;
  const unsigned diagonal_tile = has_diagonal
      ? (global_query - kv_start) / DIM : 0u;
  ...
  const unsigned future_full_tiles =
  ...
  if (has_diagonal) {
    ...
    vpu_fa_group_write_gp(
        ...
        vpu_fa_distributed_matrix_address(
            false, q_rows, score_tiles, first_row, diagonal_tile,
            config->gemmini_mask));
    vpu_fa_group_write_gp(
        group_id, VPU_FA_GP_MASK_ROW,
  ...
  if (future_full_tiles != 0u || future_tail) {
    ...
    vpu_fa_group_write_gp(
        ...
        vpu_fa_distributed_matrix_address(
            false, q_rows, score_tiles, first_row, first_future_tile,
            config->gemmini_mask));
  }
  ...
  if (has_diagonal) {
    ...
    vpu_fa_group_vector(group_id, VPU_OP_V_ADD_VV,
                        VPU_FA_GP_VECTOR, VPU_FA_GP_VECTOR,
                        VPU_FA_GP_MASK_ROW);
  }
  if (future_full_tiles != 0u || future_tail) {
  ...
  if (has_diagonal) {
    vpu_fa_group_addi_gp(group_id, VPU_FA_GP_SCORE_ROW,
                         VPU_FA_GP_SCORE_ROW, DIM);
    vpu_fa_group_addi_gp(group_id, VPU_FA_GP_MASK_ROW,
                         VPU_FA_GP_MASK_ROW, DIM);
  }
  if (future_full_tiles != 0u || future_tail)
    vpu_fa_group_addi_gp(group_id, VPU_FA_GP_FUTURE_ROW,
                         VPU_FA_GP_FUTURE_ROW, DIM);
...
static inline void vpu_fa_issue_unmasked_row_run(
    const vpu_flashattention_config_t *config,
    vpu_flashattention_stats_t *stats, unsigned group_id,
    unsigned q_rows, unsigned kv_rows, unsigned first_row,
    unsigned row_count, bool first_block) {
  const unsigned score_tiles = vpu_fa_ceil_div(kv_rows, DIM);
  const unsigned full_score_tiles = kv_rows / DIM;
  const unsigned tail_score_columns = kv_rows % DIM;
  const unsigned value_tiles =
  ...
  vpu_fa_group_write_gp(
      group_id, VPU_FA_GP_SCORE_ROW,
      vpu_fa_distributed_matrix_address(
          false, q_rows, score_tiles, first_row, 0u,
          config->gemmini_mask));
  if (!first_block)
    vpu_fa_group_write_gp(
        group_id, VPU_FA_GP_OUTPUT_ROW,
        vpu_fa_distributed_matrix_address(
            true, q_rows, value_tiles, first_row, 0u,
            config->gemmini_mask));
  vpu_fa_group_write_gp(group_id, VPU_FA_GP_STATE, first_row);
  vpu_fa_group_write_gp(group_id, VPU_FA_GP_STATE_L,
                        state_l_base + first_row);
  ...
  if (first_block) {
    ...
    vpu_fa_group_state_load_current(group_id, VPU_FA_FP_M_OLD,
                                    VPU_FA_GP_STATE);
  ...
  vpu_fa_group_score_max_stream(group_id, full_score_tiles,
                                tail_score_columns);
  vpu_fa_group_scalar(group_id, VPU_OP_S_MAX_FP,
                      VPU_FA_FP_M_NEW, VPU_FA_FP_M_OLD,
                      VPU_FA_FP_M_NEW);
  vpu_fa_group_scalar(group_id, VPU_OP_S_SUB_FP,
                      VPU_FA_FP_ALPHA, VPU_FA_FP_M_OLD,
                      VPU_FA_FP_M_NEW);
  vpu_fa_group_scalar(group_id, VPU_OP_S_EXP_FP,
                      VPU_FA_FP_ALPHA, VPU_FA_FP_ALPHA, 0u);
  vpu_fa_group_write_fp(group_id, VPU_FA_FP_P_SUM, 0.0f);
  vpu_fa_group_score_exp_stream(group_id, full_score_tiles,
                                tail_score_columns);
  vpu_fa_group_scalar(group_id, VPU_OP_S_MUL_FP,
                      VPU_FA_FP_L_OLD, VPU_FA_FP_L_OLD,
                      VPU_FA_FP_ALPHA);
  vpu_fa_group_scalar(group_id, VPU_OP_S_ADD_FP,
                      VPU_FA_FP_L_OLD, VPU_FA_FP_L_OLD,
                      VPU_FA_FP_P_SUM);
  vpu_fa_group_state_store_current(group_id, VPU_FA_GP_STATE,
                                   VPU_FA_FP_M_NEW);
  ...
  vpu_fa_group_addi_gp(group_id, VPU_FA_GP_STATE,
                       VPU_FA_GP_STATE, 1u);
  vpu_fa_group_addi_gp(group_id, VPU_FA_GP_STATE_L,
                       VPU_FA_GP_STATE_L, 1u);
  vpu_fa_group_loop_end(group_id, VPU_FA_GP_ROW_LOOP);
}

static inline void vpu_fa_issue_qk_vpu(
    const vpu_flashattention_config_t *config,
  ...
  for (unsigned row = 0u; row < q_rows;) {
    const unsigned global_query =
        (unsigned)config->query_base + q_start + row;
    const unsigned rows_to_local_boundary = DIM - row % DIM;
    const unsigned rows_to_mask_boundary = global_query < kv_start
        ? kv_start - global_query
        : DIM - (global_query - kv_start) % DIM;
    const unsigned rows_left = q_rows - row;
    unsigned row_run = rows_left < rows_to_local_boundary
        ? rows_left : rows_to_local_boundary;
    if (rows_to_mask_boundary < row_run) row_run = rows_to_mask_boundary;
    vpu_fa_issue_causal_mask_row_run(
        config, stats, group_id, q_rows, kv_start, kv_rows,
        row, row_run, global_query);
    row += row_run;
  }
  for (unsigned row = 0u; row < q_rows;) {
    ...
    const unsigned row_run = rows_left < rows_to_tile_boundary
        ? rows_left : rows_to_tile_boundary;
    vpu_fa_issue_unmasked_row_run(
        config, stats, group_id, q_rows, kv_rows, row, row_run,
        first_block);
    row += row_run;
  }
...
static inline void vpu_fa_issue_final_normalize(
    const vpu_flashattention_config_t *config,
  ...
  for (unsigned row = 0u; row < q_rows;) {
    ...
    vpu_fa_group_write_gp(
        group_id, VPU_FA_GP_OUTPUT_ROW,
        vpu_fa_distributed_matrix_address(
            true, q_rows, value_tiles, row, 0u,
            config->gemmini_mask));
    vpu_fa_group_write_gp(group_id, VPU_FA_GP_STATE_L,
                          state_l_base + row);
    ...
    vpu_fa_group_state_load_current(group_id, VPU_FA_FP_L_OLD,
                                    VPU_FA_GP_STATE_L);
    vpu_fa_group_scalar(group_id, VPU_OP_S_RECI_FP,
                        VPU_FA_FP_L_OLD, VPU_FA_FP_L_OLD, 0u);
...
static inline void vpu_fa_enqueue_output_store(
    const vpu_flashattention_config_t *config,
  ...
  for (unsigned row = 0u; row < q_rows; ++row) {
    const uint32_t first_vector = vpu_fa_distributed_matrix_address(
        true, q_rows, value_tiles, row, 0u, config->gemmini_mask);
    const uint32_t first_host = (uint32_t)(
        ((size_t)q_start + row) * config->output_stride);
    if (full_tiles >= 2u) {
      vpu_set_vl(DIM);
      ...
      vpu_h_store_v(VPU_FA_GP_VECTOR, VPU_FA_GP_HOST_OFFSET,
                    VPU_FA_H_OUTPUT);
    ...
    if (full_tiles < value_tiles) {
      ...
      vpu_write_gp(
          VPU_FA_GP_VECTOR,
          vpu_fa_distributed_matrix_address(
              true, q_rows, value_tiles, row, full_tiles,
              config->gemmini_mask));
...
static inline void vpu_fa_initialize_causal_mask(float *workspace) {
  for (unsigned row = 0u; row < DIM; ++row)
    for (unsigned column = 0u; column < DIM; ++column)
      workspace[row * DIM + column] =
          column <= row ? 0.0f : -INFINITY;
}

static inline void vpu_fa_prefetch_causal_mask(
    const vpu_flashattention_config_t *config) {
  vpu_write_h(VPU_FA_H_CAUSAL_MASK,
              (uintptr_t)config->causal_mask_workspace);
...
static inline void vpu_fa_flush_gemminis(unsigned mask) {
  for (unsigned custom = 0u; custom < 4u; ++custom)
    if (((mask >> custom) & 1u) != 0u) vpu_fa_gemmini_flush(custom);
}
...
static inline vpu_flashattention_status_t vpu_fa_preflight_jobs(
    const vpu_flashattention_config_t *config,
    const vpu_flashattention_plan_t *plan) {
  for (size_t q_start = 0u; q_start < config->query_rows;) {
    const unsigned q_rows =
        config->query_rows - q_start < plan->q_rows
            ? (unsigned)(config->query_rows - q_start) : plan->q_rows;
    ...
    for (size_t kv_start = 0u; kv_start < causal_key_end;) {
      const unsigned kv_rows = causal_key_end - kv_start < plan->kv_rows
          ? (unsigned)(causal_key_end - kv_start) : plan->kv_rows;
      vpu_fa_matmul_job_t job;
      if (!vpu_fa_matmul_job_prepare(
              &job, config, VPU_FA_MATMUL_QK,
              (unsigned)q_start, q_rows, (unsigned)kv_start, kv_rows))
...
static inline vpu_flashattention_result_t vpu_flashattention_auto(
    const vpu_flashattention_config_t *source) {
  ...
  if (result.status != VPU_FLASHATTENTION_OK) return result;
  result.status = vpu_fa_preflight_jobs(&config, &result.plan);
  if (result.status != VPU_FLASHATTENTION_OK) return result;

  for (size_t q_start = 0u; q_start < config.query_rows;) {
    const unsigned q_rows =
        config.query_rows - q_start < result.plan.q_rows
            ? (unsigned)(config.query_rows - q_start) : result.plan.q_rows;
    const size_t query_block_end = config.query_base + q_start + q_rows;
    const size_t causal_key_end = query_block_end < config.sequence
        ? query_block_end : config.sequence;
  ...
  for (size_t q_start = 0u; q_start < config.query_rows;) {
    ...
    for (size_t kv_start = 0u; kv_start < causal_key_end;) {
      const unsigned kv_rows = causal_key_end - kv_start < result.plan.kv_rows
          ? (unsigned)(causal_key_end - kv_start) : result.plan.kv_rows;
      const bool last_causal_block =
          kv_start + kv_rows == causal_key_end;

      vpu_fa_matmul_job_t qk_job;
      (void)vpu_fa_matmul_job_prepare(
          &qk_job, &config, VPU_FA_MATMUL_QK,
          (unsigned)q_start, q_rows, (unsigned)kv_start, kv_rows);
      ...
      vpu_fa_issue_qk_vpu(
          ...
          qk_job.last_group_id);

      vpu_fa_matmul_job_t pv_job;
      (void)vpu_fa_matmul_job_prepare(
          &pv_job, &config, VPU_FA_MATMUL_PV,
          (unsigned)q_start, q_rows, (unsigned)kv_start, kv_rows);
      ...
      vpu_fa_issue_final_normalize(
          &config, &result.stats, q_rows, last_causal_block,
          pv_job.last_group_id);
      kv_start += kv_rows;
    }
  ...
  result.status = (result.vpu_status & VPU_STATUS_ERROR_MASK) != 0u
      ? VPU_FLASHATTENTION_VPU_ERROR : VPU_FLASHATTENTION_OK;
  return result;
}
