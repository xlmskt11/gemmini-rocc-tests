...
// made
static void printMatrix_dynamic(const elem_t* m, size_t rows, size_t cols) {
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      ...
      printf("%d ", m[idx]);
#else
      printf("%x ", elem_t_to_elem_t_bits(m[idx]));
#endif
    }
    printf("\n");
  }
...
// made
static int is_equal_dynamic(const elem_t* x, const elem_t* y, size_t rows, size_t cols) {
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      size_t idx = i * cols + j;
#ifndef ELEM_T_IS_FLOAT
      if (x[idx] != y[idx])
#else
      bool isnanx = elem_t_isnan(x[idx]);
      bool isnany = elem_t_isnan(y[idx]);
      if (x[idx] != y[idx] && !(isnanx && isnany))
#endif
        return 0;
    }
  }
